...
from config import gc_root
gc_train_root = os.path.join(gc_root, 'train')
gc_test_root = os.path.join(gc_root, 'test')
...
class GCBase(torchvision.datasets.ImageFolder):

    def __init__(self, root, transform,is_train=True,size_transform=None):

        super(GCBase, self).__init__(root, transform)
...
def get_gc_datasets(train_transform, test_transform,size_transform=None, train_classes=range(80),
    ...
    whole_training_set = GCBase(root=gc_train_root, transform=train_transform,size_transform = size_transform)
    ...
    test_dataset = GCBase(root=gc_test_root, transform=test_transform)
...
if __name__ == '__main__':
    ...
    x = get_gc_datasets(None, None, split_train_val=False,size_transform=None,
